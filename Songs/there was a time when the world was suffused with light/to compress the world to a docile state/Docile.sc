...
Docile : IM_Module {
	...
	var mainOn, mainOff, kickArray, noisiesArray;
	...
	*new {
	...
	prInit { | modularOutBus, moogInBus, modularInBus, moogDeviceName, moogPortName |
		...
		server.waitForBoot {
			...
			// location of samples
			kicksPath = "~/Library/Application Support/SuperCollider/Extensions/prm/Songs/there was a time when the world was suffused with light/to compress the world to a docile state/samples/Kick/".standardizePath;
			kicksArray = (kicksPath ++ "*").pathMatch;

			while({ try { mixer.isLoaded } != true }, { 0.001.wait; });

			main = Docile_Main.new(mixer.chanStereo(0), modularOutBus, modularInBus, group, \addToHead);
			...
			moog = Docile_Moog.new(moogInBus, mixer.chanStereo(3), moogDeviceName, moogPortName, group, \addToHead);
			...
			//this.prMakeMIDIFuncs(sequencer);
	...
	makeMIDIFuncs { | sequencer |
		...
		mainOn = MIDIFunc.noteOn({ main.playLoop }, 60, mainChan, seq);
		mainOff = MIDIFunc.noteOff({ main.stopLoop; }, 60, mainChan, seq);
		kickArray = Array.fill(6, { | i |
			MIDIFunc.noteOn({ | vel | kick.playSampleOneShot(i, vel.ccdbfs) }, i+60, kickChan, seq); });
		noisiesArray = Array.fill(8, { | i |
			MIDIFunc.noteOn({ | vel | noisies.sampler.playSampleOneShot(i, vel.ccdbfs, 1, 0, 0.1, rrand(-1, 1)); },
				i+60, noisiesChan, seq); });
		midiFuncsLoaded = true;
	}

	freeMIDIFuncs {
		mainOn.free; mainOff.free;
		kickArray.do({ | i | i.free; }); noisiesArray.do({ | i | i.free; });
