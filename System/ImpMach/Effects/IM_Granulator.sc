...
adding in variables for tracking parameters
finally
prm
*/
...
IM_Granulator : IM_Processor {
	...
	var <isLoaded;
	var <crossfade;
	var <grainDurLow, <grainDurHigh, <trigRate, <panLow, <panHigh;
	var <grainEnvelope, <sync, <rateLow, <rateHigh;
	var <posLow, <posHigh, <filterCutoff;
	...
	*new {
	...
	setPos { | low = 0.2, high = 0.6 |
		this.setPosLow(low);
		this.setPosHigh(high);
	}
	setPosLow { | pos = 0.2 |
		posLow = pos;
		synth.set(\posLow, posLow);
	}
	setPosHigh { | pos = 0.6 |
		posHigh = pos;
		synth.set(\posHigh, posHigh);
	}
