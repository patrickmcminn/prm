...
GrainCloud2 : IM_Module {
	...
	*new { | outBus = 0, send0Bus, send1Bus, send2Bus, send3Bus, relGroup = nil, addAction = \addToHead |
	...
	prAddSynthDefs{
		...
		SynthDef(\gaborRect, {
			| outBus, amp = 0.1, freq = 440, width = 0.5, sustain = 0.01, pan = 0, cutoff = 5000, rq = 1 |
			var snd = PulseDPW.ar(freq, width);
			var filter = RLPF.ar(snd, cutoff, rq);
			var amp2 = amp * AmpComp.ir(freq.max(50)) * 0.5;
			var env = EnvGen.ar(Env.sine(sustain, amp2), doneAction: 2);
			OffsetOut.ar(outBus, Pan2.ar(filter * env, pan));
		...
		SynthDef(\gaborWideRect, {
			| outBus, amp = 0.1, freq = 440, envWidth = 0.5, sustain = 0.01, pan = 0, width = 0.5, cutoff = 5000, rq = 1 |
			var holdT = sustain * envWidth;
			var fadeT = 1 - width * sustain * 0.5;
			var snd = PulseDPW.ar(freq, width);
			var filter = RLPF.ar(snd, cutoff, rq);
			var amp2 = amp * AmpComp.ir(freq.max(50)) * 0.5;
	...
	setWidth { | low = 0.5, high = 0.5 |
		this.setWidthLow(low);
		this.setWidthHigh(high);
	}
