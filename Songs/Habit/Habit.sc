...
Habit : IM_Module {
	...
	*new {
		|
		outBus, micIn, inArray, outArray, digiArray, moogDeviceName, moogPortName, sequencer, delay,
		...
		^super.new(5, outBus, send0Bus, send1Bus, send2Bus, send3Bus, false,
			relGroup, addAction).prInit(micIn, inArray, outArray, digiArray, moogDeviceName, moogPortName, sequencer, delay);
	}

	prInit {
		|
		micIn, inArray, outArray, digiArray, moogDeviceName, moogPortName, sequencer, delay
		|
		...
		server.waitForBoot {
			...
			*/
			...
			terr = IM_HardwareIn.newStereo(digiArray[0], mixer.chanStereo(2), group, \addToHead);
