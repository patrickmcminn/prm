Subtractive_Voice {
	...
	*newCustomOut { | outBus, freq = 220, vol = -12, subtractive, group = nil, addAction = 'addToHead' |
		^super.new.prInitCustomOut(outBus, freq, vol, subtractive, group, addAction);
	}
	...
	prInitCustomOut { | outBus, freq, vol, subtractive, group, addAction |
		server = Server.default;
		parent = subtractive;
		isFree = false;
		{
			watcher = server.makeBundle(false, {
				synth = Synth(\prm_Subtractive_Voice, [
					\outBus, outBus, \lfo2InBus, parent.lfoBus, \amp, vol.dbamp, \freq, freq, \gate, 1,
