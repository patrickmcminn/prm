  ...
  *newStereo {
  ...
  prAddSynthDefs {
    SynthDef(\prm_SamplePlayer_Stereo_ADSR, {
      ...
      tremSine = SinOsc.ar(tremFreq).range(0, 1);
      tremSaw = LFSaw.ar(tremFreq, 1).range(0, 1);
      tremRevSaw = LFSaw.ar(tremFreq, 1).range(-1, 0) * -1;
      tremRect = LFPulse.ar(tremFreq, 0, tremPulseWidth).range(0, 1);
      tremNoise = LFNoise2.ar(tremFreq).range(0, 1);
      tremSampleAndHold = LFNoise0.ar(tremFreq).range(0, 1);
